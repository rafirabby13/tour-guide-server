...
router.post("/create-tour", fileUploader.upload.array("files"), auth(UserRole.GUIDE), (req, res, next) => {
...
router.patch("/update-tour/:id", fileUploader.upload.array("files"), auth(UserRole.GUIDE), (req, res, next) => {
    req.body = updateTourZodSchema.shape.body.parse(JSON.parse(req.body.data));
    return TourController.updateTour(req, res, next);
});
router.patch("/update-tour-status", auth(UserRole.ADMIN), TourController.updatetourStatus);
router.delete("/delete/:id", auth(UserRole.GUIDE), TourController.deleteTour);
router.get("/my/tours", auth(UserRole.GUIDE), TourController.getMyTours);
